...
#define MarginButton [UIApplication sharedApplication].keyWindow.frame.size.height - 140
...
- (void)finishMovie:(UIButton *)sender {
    NSLog(@"完成按钮被点击");
    if (self.seconds == self.recordTime && sender != nil) {
        self.seconds = 0;
        return;
    }
...
- (void)timerMethod {
    ...
    if (self.seconds == self.recordTime) {
...
//    NSLog(@"%@", [self.fileManager subpathsAtPath:path]);
...
- (void)saveVideoToCameraRoll{
    ...
    if (ios8Later) {
        [[PHPhotoLibrary sharedPhotoLibrary] performChanges:^{
            [PHAssetChangeRequest creationRequestForAssetFromVideoAtFileURL:[NSURL URLWithString:self.compressVideoFilePath]];
...
//    NSLog (@"file size: %f -- %lf", (unsigned long long)[outputFileAttributes fileSize]/1024.00 /1024.00, fileSize);
    return fileSize;
}
...
//    NSLog(@"%@",[_fileManager subpathsAtPath:path]);
    return outPutPath;
}
...
- (void)setupUp {
    ...
    cancelBtn.frame = CGRectMake((ScreenWidth / 4 - 40) / 2, MarginButton, 40, 40);
    
    // 翻转摄像头
    UIButton *turnBtn = [UIButton new];
    [self.view addSubview:turnBtn];
    [turnBtn setImage:[UIImage imageNamed:@"icon_changeCamera"] forState:UIControlStateNormal];
    ...
    turnBtn.frame = CGRectMake(ScreenWidth - (ScreenWidth / 4 - 40) / 2 - 40, MarginButton, 40, 40);
...
- (void)setupDownButton {
    ...
    [repeatRecordBtn setImage:[UIImage imageNamed:@"icon_return"] forState:UIControlStateNormal];
    ...
    repeatRecordBtn.frame = CGRectMake((ScreenWidth / 4 - 40) / 2, MarginButton, 40, 40);
    
    // 发送视频
    UIButton *sendVideoBtn = [UIButton new];
    [self.view addSubview:sendVideoBtn];
    [sendVideoBtn setImage:[UIImage imageNamed:@"icon_success"] forState:UIControlStateNormal];
    ...
    sendVideoBtn.frame = CGRectMake(ScreenWidth - (ScreenWidth / 4 - 40) / 2, MarginButton, 40, 40);
...
- (void)setupVideotapButton {
    ...
    btn.frame = CGRectMake((ScreenWidth - _recordBtnWidth) / 2, MarginButton, _recordBtnWidth, _recordBtnWidth);
    
    CAShapeLayer *maskLayer = [CAShapeLayer new];
    _beginRect = CGRectMake((ScreenWidth - _recordBtnWidth) / 2, MarginButton, _recordBtnWidth, _recordBtnWidth);
    ...
    smallCircleLayer.path = [UIBezierPath bezierPathWithOvalInRect:CGRectMake((ScreenWidth - 50) / 2, MarginButton + 10, 50, 50)].CGPath;
...
//    NSLog(@"%@",_captureSession.sessionPreset);
    // 2. 创建输入设备
    self.cameraPosition = AVCaptureDevicePositionBack;
...
- (CAShapeLayer *)animateShapLayer {
    if (_animateShapLayer == nil) {
        ...
        _animateShapLayer.strokeColor = [UIColor greenColor].CGColor;
    }
